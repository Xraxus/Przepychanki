...
def check_simple():
    if session.get('key') and 'games' in globals():     # check if session key exists and if games dictionary is defined in the global namespace
        return True
    return False

def check():
    if session.get('key') and 'games' in globals() and session.get('key') in games:   # check if session key exists, if games dictionary is defined in the global namespace, and if the session key exists as a key in the games dictionary
        return True
    return False

def check_local_phase(phase_index):
    if games[session['key']].current_phase != phase_index:      # check if the current phase of the game is not equal to the given phase index
        if games[session['key']].current_phase == '0':          # redirect to the corresponding phase based on the current phase of the game
...
def check_multiplayer_phase(phase_index):
    ...
    if multiplayer_games[session['game_uuid']].current_phase != phase_index:
        ...
        if multiplayer_games[session['game_uuid']].current_phase == '0':
            ...
            return redirect('/', code=302)  # redirect to the home page if the current phase is not recognized
    else:
